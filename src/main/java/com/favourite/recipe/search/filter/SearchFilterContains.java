...
public class SearchFilterContains implements SearchFilter {
    ...
    public boolean match(SearchOperation opt) {
        return opt == SearchOperation.CN;
    }

    @Override
    public Predicate apply(CriteriaBuilder cb, FilterKey filterKey, String filterValue, Root<Recipe> root, Join<Object, Object> subRoot) {
    ...
    private Predicate getPredicate(CriteriaBuilder cb, Path<Object> subRoot, String filterValue) {
        return cb.like(cb.lower(subRoot.as(String.class)), "%" + filterValue + "%");
    }
